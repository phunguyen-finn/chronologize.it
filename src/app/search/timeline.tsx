...
import { CheckOutlined, CloseOutlined, CopyOutlined, ExpandAltOutlined, ShareAltOutlined } from '@ant-design/icons';
        ...
        <div>
            <div className="fixed top-0 right-0 p-5 z-20">
                <button
                    ...
                    className="disabled:cursor-not-allowed cursor-pointer flex items-center gap-x-[10px] justify-center dark:bg-white bg-black text-white dark:text-black p-2 rounded-lg hover:shadow-xl ease-in-out duration-100 px-2"
                >
                                ...
                                timeline.markers.map((marker: Marker, index: number) => {
                                    return <React.Fragment key={index}>
                                        {loading == index && Array(3).fill(null).map((_, i) =>
                                            <Segment key={i} position={(index + i) % 2 ? "top" : "bottom"}>
                                                ...
                                                >
                                                    <button
                                                        style={{ opacity: 1 }}
                                                        className="origin-center  h-7 w-7 flex items-center opacity-0 justify-center relative bg-black/[0.5] dark:bg-white/[0.5] group-hover:opacity-100 hover:scale-150 hover:bg-black dark:hover:bg-white sm:hover:rotate-45 ease-in-out duration-[200ms] text-white sm:w-10 sm:h-10 rounded-full">
                                                        {
                                                            loading == index
                                                                ...
                                                                : <ExpandAltOutlined color={theme === 'light' ? 'white' : 'black'} />
                                                        }
                                                    </button>
                                                </div>)}
                                            </Segment>)}
                                        <Segment marker={marker} handleClick={() => handleClick(index)} position={index % 2 ? "top" : "bottom"}>
                                            ...
                                            >
                                                <button
                                                    className="origin-center hover:-rotate-45 h-7 w-7 flex items-center opacity-100 sm:opacity-0 justify-center relative dark:bg-white/[0.5] bg-black/[0.5] group-hover:opacity-100 hover:scale-150 hover:bg-black dark:hover:bg-white sm:hover:rotate-45 ease-in-out duration-[200ms] text-white sm:w-10 sm:h-10 rounded-full">
                                                    {
                                                        loading == index
                                                            ? <svg aria-hidden="true" className="inline w-4 h-4 text-black animate-spin dark:text-white fill-white dark:fill-black" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" />
                                                            ...
                                                            : <ExpandAltOutlined style={{ color: theme === 'light' ? 'white' : 'black' }} />
                                                    }
                                                </button>
                                            </div>
                                            }
            ...
            <Modal
                ...
                classNames={{ wrapper: 'dark:bg-gray-500/50' }}
            >
            ...
            >
                ...
                >
                    <Input disabled value={shareUrl} rootClassName='disabled:dark:text-white' />
                    <CopyToClipboard text={shareUrl} onCopy={() => setCopied(true)} >
                        <button
                            style={{ borderRadius: '0px 5px 5px 0px' }}
                            className='flex items-center gap-x-[10px] justify-center dark:bg-white bg-black dark:text-black text-white p-2 hover:shadow-xl ease-in-out duration-100 px-2' >
